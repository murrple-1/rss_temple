...
class DaemonTestCase(TestCase):
    ...
    def test_render_regular(self):
        ...
        models.NotifyEmailQueueEntryRecipient.objects.create(
            type=models.NotifyEmailQueueEntryRecipient.TYPE_TO, email='to@test.com', entry=email_entry)
        models.NotifyEmailQueueEntryRecipient.objects.create(
            type=models.NotifyEmailQueueEntryRecipient.TYPE_CC, email='cc@test.com', entry=email_entry)
        models.NotifyEmailQueueEntryRecipient.objects.create(
            type=models.NotifyEmailQueueEntryRecipient.TYPE_BCC, email='bcc@test.com', entry=email_entry)
    ...
    def test_render_smtpdisconnected(self):
        ...
        with self.assertLogs(logger(), logging.ERROR) as cm:
            ...
            self.assertGreaterEqual(len(
                [line for line in cm.output if re.search(r'SMTP server disconnected', line)]), 1)
    ...
    def test_render_generalerror(self):
        ...
        with self.assertLogs(logger(), logging.ERROR) as cm:
            ...
            self.assertGreaterEqual(len([line for line in cm.output if re.search(
                r'error sending email notification', line)]), 1)
    ...
    def test_render_countwarning(self):
        ...
        models.NotifyEmailQueueEntryRecipient.objects.create(
            type=models.NotifyEmailQueueEntryRecipient.TYPE_TO, email='to@test.com', entry=email_entry1)

        email_entry2 = models.NotifyEmailQueueEntry.objects.create(
            subject='Subject2', plain_text='Some Text2', html_text='<b>Some Text2</b>')
        models.NotifyEmailQueueEntryRecipient.objects.create(
            type=models.NotifyEmailQueueEntryRecipient.TYPE_TO, email='to@test.com', entry=email_entry2)
        ...
        with self.assertLogs(logger(), logging.WARNING) as cm:
            ...
            self.assertGreaterEqual(len([line for line in cm.output if re.search(
                r'still more email queue entries than expected', line)]), 1)
