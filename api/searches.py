...
_search_fns = {
    ...
    "feed": {
        ...
        "publishedAt": lambda context, search_obj: Q(
            published_at__range=DateTimeRange.convertto(search_obj)
        ),
        "publishedAt_exact": lambda context, search_obj: Q(
            published_at=DateTime.convertto(search_obj)
        ),
        "publishedAt_delta": lambda context, search_obj: Q(
            published_at__range=DateTimeDeltaRange.convertto(search_obj)
        ),
        "updatedAt": lambda context, search_obj: Q(
            updated_at__range=DateTimeRange.convertto(search_obj)
        ),
        "updatedAt_exact": lambda context, search_obj: Q(
            updated_at=DateTime.convertto(search_obj)
        ),
        "updatedAt_delta": lambda context, search_obj: Q(
            updated_at__range=DateTimeDeltaRange.convertto(search_obj)
        ),
        "subscribed": lambda context, search_obj: Q(
            is_subscribed=Bool.convertto(search_obj)
        ),
        "customTitle": lambda context, search_obj: Q(
            custom_title__icontains=search_obj
        ),
        "customTitle_exact": lambda context, search_obj: Q(
            custom_title__iexact=search_obj
        ),
        "customTitle_null": lambda context, search_obj: Q(
            custom_title__isnull=Bool.convertto(search_obj)
        ),
        "calculatedTitle": lambda context, search_obj: Q(title__icontains=search_obj)
        | Q(custom_title__icontains=search_obj),
        "calculatedTitle_exact": lambda context, search_obj: Q(title__iexact=search_obj)
        | Q(custom_title__iexact=search_obj),
    },
    "feedentry": {
        "uuid": lambda context, search_obj: Q(uuid__in=UuidList.convertto(search_obj)),
        "feedUuid": lambda context, search_obj: Q(
            feed_id__in=UuidList.convertto(search_obj)
        ),
        "feedUrl": lambda context, search_obj: Q(feed__feed_url=search_obj),
        "createdAt": lambda context, search_obj: Q(
            created_at__range=DateTimeRange.convertto(search_obj)
        ),
        "createdAt_exact": lambda context, search_obj: Q(
            created_at=DateTime.convertto(search_obj)
        ),
        "createdAt_delta": lambda context, search_obj: Q(
        ...
        "authorName_exact": lambda context, search_obj: Q(
            author_name__iexact=search_obj
        ),
        "subscribed": _feedentry_subscribed,
        "isRead": _feedentry_is_read,
        "isFavorite": _feedentry_is_favorite,
        "readAt": lambda context, search_obj: Q(
            uuid__in=models.ReadFeedEntryUserMapping.objects.filter(
                user=context.request.user,
                read_at__range=DateTimeRange.convertto(search_obj),
            )
        ),
        "readAt_exact": lambda context, search_obj: Q(
            uuid__in=models.ReadFeedEntryUserMapping.objects.filter(
                user=context.request.user, read_at=DateTime.convertto(search_obj)
            )
        ),
        "readAt_delta": lambda context, search_obj: Q(
            uuid__in=models.ReadFeedEntryUserMapping.objects.filter(
                user=context.request.user,
    ...
    if "WhereClause" in parse_results and "WhereExpressionExtension" in parse_results:
        ...
        if "AndOperator" in where_expression_extension:
            return _handle_parse_result(
                context, where_clause, object_search_fns
            ) & _handle_parse_result(
                context, where_expression_extension, object_search_fns
            )
            ...
            return _handle_parse_result(context, where_clause, object_search_fns)
        ...
        return Q(
            _handle_parse_result(
                context, parse_results["ParenthesizedExpression"], object_search_fns
            )
        )
