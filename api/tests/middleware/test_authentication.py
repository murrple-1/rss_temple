...
class AuthenticationTestCase(TestCase):
    def test_succeed_authentication(self):
        with self.settings(
            REALM="Test Realm", AUTHENTICATION_DISABLE=[(r"^/test/?$", ["POST"])]
        ):
    ...
    def test_none_disable(self):
        with self.settings(REALM="Test Realm", AUTHENTICATION_DISABLE=None):
            middleware = authentication.AuthenticationMiddleware(
                lambda request: HttpResponse()
            )
    ...
    def test_none_realm(self):
        with self.settings(REALM=None, AUTHENTICATION_DISABLE=None):
            middleware = authentication.AuthenticationMiddleware(
                lambda request: HttpResponse()
            )
